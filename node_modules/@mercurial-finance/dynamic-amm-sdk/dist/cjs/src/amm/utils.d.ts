...
import { AccountInfo as AccountInfoInt } from '@solana/spl-token';
import { AccountInfo, Connection, PublicKey, TransactionInstruction } from '@solana/web3.js';
import { SwapCurve } from './curve';
import { ConstantProductCurve, DepegLido, DepegMarinade, DepegNone, PoolInformation, PoolState, StableSwapCurve, SwapQuoteParam, SwapResult, TokenMultiplier } from './types';
...
export declare const getOrCreateATAInstruction: (tokenMint: PublicKey, owner: PublicKey, connection: Connection, allowOwnerOffCurve?: boolean) => Promise<[PublicKey, TransactionInstruction?]>;
export declare const wrapSOLInstruction: (from: PublicKey, to: PublicKey, amount: bigint) => TransactionInstruction[];
export declare const unwrapSOLInstruction: (owner: PublicKey, allowOwnerOffCurve?: boolean) => Promise<TransactionInstruction | null>;
export declare const deserializeAccount: (data: Buffer | undefined) => AccountInfoInt | undefined;
...
export declare function derivePoolAddress(connection: Connection, tokenInfoA: TokenInfo, tokenInfoB: TokenInfo, isStable: boolean, opt?: {
    programId?: string;
}): PublicKey;
/**
 ...
 */
export declare function checkPoolExists(connection: Connection, tokenInfoA: TokenInfo, tokenInfoB: TokenInfo, isStable: boolean, opt?: {
    programId: string;
...
export declare function encodeCurveType(curve: StableSwapCurve | ConstantProductCurve): 1 | 0;
